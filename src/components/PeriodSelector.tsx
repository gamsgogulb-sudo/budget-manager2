...
export default function PeriodSelector({ 
            ...
            {quickActions.map((btn) => (
              <button 
                ...
                className={cn(
                  ...
                  period === btn.id && "border-[#007AFF] text-[#007AFF] bg-blue-50"
                )}
              >
                {btn.label}
              </button>
  ...
  return (
    <div className={cn("space-y-4", className)}>
      <div className="flex bg-[#F5F5F7] p-1 rounded-xl w-full overflow-x-auto no-scrollbar">
        {presets.map((p) => (
          <button
            key={p.id}
            onClick={() => onChangePeriod(p.id as PeriodType)}
            className={cn(
              "flex-1 px-4 py-1.5 rounded-lg text-xs font-bold transition-all whitespace-nowrap",
      ...
      <AnimatePresence>
        {period === 'custom' && (
          ...
          >
            <div className="flex flex-wrap items-center gap-3 p-4 bg-[#F5F5F7] rounded-2xl">
              <div className="flex-1 min-w-[140px]">
                ...
                <input 
                  ...
                  className="w-full bg-white border-none rounded-xl px-4 py-2.5 text-sm font-semibold focus:ring-2 focus:ring-[#007AFF]/20 transition-all"
                />
              </div>
